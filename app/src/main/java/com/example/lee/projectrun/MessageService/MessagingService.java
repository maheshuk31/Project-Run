package com.example.lee.projectrun.MessageService;
...
import com.example.lee.projectrun.Actions.LoggingIn;
import com.example.lee.projectrun.Actions.ChatMessage;
import com.example.lee.projectrun.R;
import com.example.lee.projectrun.MainComponentsInformation.SocketConnection;
import com.example.lee.projectrun.appInterfaces.IUserMessageNetworkManager;
import com.example.lee.projectrun.appInterfaces.ISocket;
...
import com.example.lee.projectrun.Controllers.XMLController;
import com.example.lee.projectrun.Controllers.MessageController;
import com.example.lee.projectrun.Controllers.DatabaseController;
 ...
 */
public class MessagingService extends Service implements IUserMessageNetworkManager, IUpdateData {
//	private NotificationManager mNM;
    ...
    ISocket socketOperator = new SocketConnection(this);
    ...
    public class IMBinder extends Binder {
        public IUserMessageNetworkManager getService() {
...
*/
    ...
    private void showNotification(String username, String msg)
    {
        ...
        i.putExtra(FriendNetworkInformation.USERNAME, username);
        i.putExtra(MessageInformation.MESSAGETEXT, msg);
    ...
    public String authenticateUser(String usernameText, String passwordText) throws UnsupportedEncodingException
    {
        ...
        if (result != null && !result.equals(LoggingIn.AUTHENTICATION_FAILED))
        {
            ...
            i.putExtra(FriendNetworkInformation.FRIEND_LIST, rawFriendList);
            sendBroadcast(i);
                ...
                public void run()
                {
                    try {
                        ...
                        if (tmp != null) {
                            ...
                            if (tmp2 != null) {
                                i2.putExtra(MessageInformation.MESSAGE_LIST, tmp2);
                                sendBroadcast(i2);
                                Log.i("friend list broadcast sent ", "");
                            }
                        }
    ...
    public void messageReceived(String username, String message)
    {
        ...
        MessageInformation msg = MessageController.checkMessage(username);
        if ( msg != null)
        {
            Intent i = new Intent(TAKE_MESSAGE);

            i.putExtra(MessageInformation.USERID, msg.userid);
            i.putExtra(MessageInformation.MESSAGETEXT, msg.messagetext);
            sendBroadcast(i);
    ...
    private void parseFriendInfo(String xml)
    {
        try
        {
            ...
            sp.parse(new ByteArrayInputStream(xml.getBytes()), new XMLController(MessagingService.this));
        }
    ...
    public void updateData(MessageInformation[] messages,FriendNetworkInformation[] friends,
                           FriendNetworkInformation[] unApprovedFriends, String userKey)
    {
